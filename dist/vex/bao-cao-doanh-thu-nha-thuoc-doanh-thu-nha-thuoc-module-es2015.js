...
let DoanhThuNhaThuocListComponent = class DoanhThuNhaThuocListComponent {
    constructor(notificationService, authService, location, dialog, apiService, route) {
    ...
    totalThanhTienKhoLe(field) {
        switch (field) {
            case 'ThanhTien':
                return this.dataDoanhThuNhaThuoc.reduce((sum, item) => sum + item.ThanhTien, 0);
            case 'TienMat':
                return this.dataDoanhThuNhaThuoc.reduce((sum, item) => sum + item.TienMat, 0);
            case 'ChuyenKhoan':
                return this.dataDoanhThuNhaThuoc.reduce((sum, item) => sum + item.ChuyenKhoan, 0);
            case 'Pos':
                return this.dataDoanhThuNhaThuoc.reduce((sum, item) => sum + item.Pos, 0);
            case 'CongNo':
                return this.dataDoanhThuNhaThuoc.reduce((sum, item) => sum + item.CongNo, 0);
        }
    }
    ngOnInit() {
    ...
    getSharedPrintForm() {
        return new Promise(resolve => {
            this.apiService.post('BaoCaoDoanhThuNhaThuoc/InDanhMucDaHetHan', this.timKiemNangCaoObj).subscribe(resultData => {
                resolve(resultData);
            });
        });
    }
    exportExcel() {
        ...
        if (self.authService.hasPermission(self.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_12__["SecurityOperation"].Process)) {
            ...
            self.apiService.postExportExcel('BaoCaoDoanhThuNhaThuoc/ExportBaoCaoDoanhThuNhaThuoc', self.gridChild._gridQueryInfo).subscribe(resultData => {
...
DoanhThuNhaThuocListComponent.ctorParameters = () => [
...
let DoanhThuNhaThuocRoutingModule = class DoanhThuNhaThuocRoutingModule {
};
...
class Search {
    constructor(TuNgayDenNgay = new SearchDateRange(), SearchString = null, HostingName = null) {
...
class DoanhThuNhaThuocVo {
    constructor(Id = null, ThanhTien = null, TienMat = null, ChuyenKhoan = null, Pos = null, CongNo = null) {
        this.Id = Id;
...
let DoanhThuNhaThuocModule = class DoanhThuNhaThuocModule {
};
...
let DoanhThuNhaThuocService = class DoanhThuNhaThuocService extends src_app_core_services_base_service__WEBPACK_IMPORTED_MODULE_2__["BaseService"] {
    constructor(apiService) {
        super(apiService);
        this.controllerName = 'BaoCaoDoanhThuNhaThuoc';
    }
};
DoanhThuNhaThuocService.ctorParameters = () => [
