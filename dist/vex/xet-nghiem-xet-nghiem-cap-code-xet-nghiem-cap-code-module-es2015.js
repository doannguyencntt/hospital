...
let XetNghiemCapCodeFileExcelErrosPopupComponent = class XetNghiemCapCodeFileExcelErrosPopupComponent {
    ...
    ngOnInit() {
        ...
        this.dataNguoiBenh.data = [...this.data.Data.filter((z) => z.IsError)];
    }
    close() {
        this.dialogRef.close();
    }
};
XetNghiemCapCodeFileExcelErrosPopupComponent.ctorParameters = () => [
...
let XetNghiemCapCodeFileExcelPopupComponent = class XetNghiemCapCodeFileExcelPopupComponent {
    constructor(dialog, dialogRef, cdRef, data, notificationService, apiService, authService) {
        this.dialog = dialog;
        this.dialogRef = dialogRef;
        this.cdRef = cdRef;
        this.data = data;
    ...
    xuLyFile() {
        ...
        if (self.authService.hasPermission(self.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_13__["SecurityOperation"].Update)) {
            if (self.fileExcel != undefined && self.fileExcel != null) {
                ...
                self.addingPage();
                self.apiService.post("XetNghiem/ImportNguoiBenhChuaCapBarcodeXetNghiem", obj).subscribe((resp) => {
                    if (resp) {
                        self.closepopupAddingData();
                        if (resp.length > 0 && resp.some((z) => z.IsError)) {
                            this.dialog.open(_xet_nghiem_cap_code_file_excel_erros_popup_xet_nghiem_cap_code_file_excel_erros_popup_component__WEBPACK_IMPORTED_MODULE_11__["XetNghiemCapCodeFileExcelErrosPopupComponent"], {
                                disableClose: false,
                                width: '1300px',
                                data: { Title: 'DS Người Bệnh Chưa Cấp Barcode Bị Lỗi.', Data: resp }
                            }).afterClosed().subscribe(() => {
                                if (resp.some((z) => !z.IsError)) {
...
XetNghiemCapCodeFileExcelPopupComponent.ctorParameters = () => [
...
let XetNghiemCapCodeListComponent = class XetNghiemCapCodeListComponent {
    constructor(authService, notificationService, router, route, location, dialog, apiService, renderer) {
        this.authService = authService;
        this.notificationService = notificationService;
        this.router = router;
        this.route = route;
    ...
    getThongTinYeuCauTiepNhan(yeuCauTiepNhanId, phienXetNghiemId = null) {
        if (yeuCauTiepNhanId) {
    ...
    getDataBarcode() {
        this.apiService.post("XetNghiem/GetListBarcodeTheoYeuCauTiepNhan", this.queryInfoBarcode).subscribe((resultData) => {
    ...
    CapVaInBarcode() {
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_29__["SecurityOperation"].Update)) {
            ...
            this.apiService.post("XetNghiem/KiemTraBarcodeDangChon", thongTinBarcode).subscribe((resultDataBarcode) => {
                var thongTinBarcode = new _xet_nghiem_cap_code_model__WEBPACK_IMPORTED_MODULE_16__["CapBarcodeTheoDichVuXetNghiem"](this.dataYeuCauTiepNhan.YeuCauTiepNhanId, this.dataYeuCauTiepNhan.BenhNhanId, resultDataBarcode.KeyId, resultDataBarcode.DisplayName, this.barcodeSoLuongIn, this.dataYeuCauTiepNhan.SoLuongThem, this.arrDichVuChon, this.dataYeuCauTiepNhan.NhanVienLayMauId, this.dataYeuCauTiepNhan.ThoiGianLayMau);
                this.apiService.put("XetNghiem/XuLyXacNhanCapBarcodeDichhVuXetNghiem", thongTinBarcode)
                    .subscribe((resultData) => {
                    this.notificationService.showSuccess(src_app_core_utilities_common_helper__WEBPACK_IMPORTED_MODULE_23__["CommonService"].format(src_app_shared_configdata_system_message__WEBPACK_IMPORTED_MODULE_30__["SystemMessage"].ActionSuccessfully, ["Cấp barcode"]));
                    this.closePopupLoadingData();
                    // trường hợp dùng lại barcode thì ẩn số lượng thêm đi
                    let soLuongInThucTe = this.barcodeSoLuongIn + (this.dataYeuCauTiepNhan.SoLuongThem == null ? 0 : (!this.dungBarcodeMoi ? 0 : this.dataYeuCauTiepNhan.SoLuongThem));
                    ...
                    // });
                    this.xuLyInBarcodeKhongHienPopup(resultDataBarcode.KeyId, resultDataBarcode.DisplayName, soLuongInThucTe);
                    this.dataYeuCauTiepNhan.NhanVienLayMauId = null;
                    this.dataYeuCauTiepNhan.ThoiGianLayMau = null;
                }, (err) => {
        ...
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_29__["SecurityOperation"].Delete)) {
            ...
            self.dialog.open(src_app_shared_component_dialogs_confirm_confirm_component__WEBPACK_IMPORTED_MODULE_31__["ConfirmComponent"], {
                ...
                if (result == "Yes") {
                    ...
                    self.apiService.put("XetNghiem/XuLyXacNhanHuyCapBarcodeDichVuChuaNhanMau", obj).subscribe(resultData => {
    ...
    xuLyInBarcodeKhongHienPopup(BarcodeNumber, BarcodeId, soLuongIn, isReload = true) {
        var self = this;
        self.showPopupLoadingData();
        let hostingName;
        ...
        let layMauXetNghiemInBarcode = new _lay_mau_xet_nghiem_lay_mau_xet_nghiem_model__WEBPACK_IMPORTED_MODULE_28__["LayMauXetNghiemInBarcode"](self.dataYeuCauTiepNhan.YeuCauTiepNhanId, BarcodeId, BarcodeNumber, self.barcodeSoLuongIn, hostingName);
        self.apiService.post("Xetnghiem/InBarcodeLayMauXetNghiem", layMauXetNghiemInBarcode).subscribe(responseHTML => {
            if (isReload) {
                this.barcodeSoLuongIn = null;
                this.xuLyRefreshQuyeryGridDichVu();
            }
    ...
    xuLyCheckDichVu(id, tenNhom, checked) {
        if (checked) {
    ...
    chonTatCaDichVuTrongGoi(event) {
        if (event) {
            ...
            lstDichVu.forEach(element => {
                ...
                this.barcodeSoLuongIn = this.arrNhomDichVuDangChon.length + 1;
            });
        }
        else {
    ...
    demNhomDangChon() {
        ...
        let length = this.arrNhomDichVuDangChon.length;
        this.barcodeSoLuongIn = length == 0 ? null : length + 1;
    }
    kiemTraAnHienGroupDichVu(tenNhom) {
    ...
    xuLyXuatExcel() {
        ...
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_29__["SecurityOperation"].Process)) {
            this.apiService.postExportExcel("XetNghiem/ExportDanhSachNguoiBenhDaCapCodeXetNghiem", this.gridDaCapCode._gridQueryInfo).subscribe(res => {
    ...
    huyChonBenhNhan(chuyenBenhNhan = false) {
        this.dataYeuCauTiepNhan = new _xet_nghiem_cap_code_model__WEBPACK_IMPORTED_MODULE_16__["ThongTinYeuCauTiepNhanXetNghiem"]();
        if (!chuyenBenhNhan) {
    ...
    xuLyXuatExcelChuaCapCode() {
        ...
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_29__["SecurityOperation"].Process)) {
            this.apiService.postExportExcel("XetNghiem/ExportDanhSachNguoiBenhChuaCapBarcodeXetNghiem", this.gridChuaCapCode._gridQueryInfo).subscribe(res => {
                let dateTimeNow = new Date();
    ...
    inNhieuBarcode() {
        if (this.timKiemNangCaoObj.TuNgayDenNgay != null
            ...
            this.apiService.post("XetNghiem/InBarcodeCuaBenhNhan", this.inBarcodeDaCapCodeBenhNhan).subscribe(htmls => {
...
XetNghiemCapCodeListComponent.ctorParameters = () => [
...
let XetNghiemCapCodeRoutingModule = class XetNghiemCapCodeRoutingModule {
};
...
let XetNghiemCapCodeModule = class XetNghiemCapCodeModule {
};
...
let XetNghiemCapCodeService = class XetNghiemCapCodeService extends src_app_core_services_base_service__WEBPACK_IMPORTED_MODULE_2__["BaseService"] {
    constructor(apiService) {
        super(apiService);
        this.controllerName = 'XetNghiem';
    }
};
XetNghiemCapCodeService.ctorParameters = () => [
