...
let LichSuXacNhanBhytDetailComponent = class LichSuXacNhanBhytDetailComponent {
    constructor(route, apiService, router, dialog, notificationService) {
        this.route = route;
    ...
    getById(id) {
        this.apiService.post('LichSuXacNhanBHYT/GetById?id=' + id).subscribe((resultData) => {
    ...
    viewDocument(idFile, type) {
        this.apiService.post('XacNhanBHYT/GetDocument?idFile=' + idFile + '&type=' + type).subscribe((result) => {
    ...
    phieuLinhThuocBenhNhanBHYT() {
        ...
        this.apiService.post("XacNhanBHYT/PhieuLinhThuocBenhNhanXacNhanBHYT?baoHiemYteId=" + this.thisId + "&hostingName=" + hostingName).subscribe(arrHtml => {
            var arrHtmls = [{
                    Html: arrHtml,
...
LichSuXacNhanBhytDetailComponent.ctorParameters = () => [
...
let LichSuXacNhanBhytListComponent = class LichSuXacNhanBhytListComponent {
    constructor(apiService, notificationService, route, dialog, authService, router) {
        this.apiService = apiService;
        this.notificationService = notificationService;
        this.route = route;
        this.dialog = dialog;
    ...
    exportExcel() {
        ...
        if (self.authService.hasPermission(self.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_12__["SecurityOperation"].Process)) {
            self.apiService.postExportExcel('LichSuXacNhanBHYT/ExportLichSuXacNhanBhyt', self.gridLichSuXacNhan._gridQueryInfo).subscribe(resultData => {
...
LichSuXacNhanBhytListComponent.ctorParameters = () => [
...
let LichSuXacNhanBhytRoutingModule = class LichSuXacNhanBhytRoutingModule {
};
...
class XacNhanBhyt {
    constructor(ThoiDiemTiepNhanTuFormat = null, ThoiDiemTiepNhanDenFormat = null, FromDate = null, ToDate = null, SearchString = null) {
...
class ThongTinBaoHiemXaHoi {
    constructor(SoThe = null, NgayHieuLucStr = null, NgayHetHanStr = null, NoiDangKyKhamBenh = null, TuyenKham = null, GiayChuyenVien = null, GiayChuyenVienId = null, GiayMiemCungTriTra = null, GiayMienCungTriTraId = null, MucHuong = 0) {
        this.SoThe = SoThe;
...
class DateTimeFilter {
    constructor(FromDate = null, ToDate = null) {
        this.FromDate = FromDate;
        this.ToDate = ToDate;
    }
}
...
let LichSuXacNhanBhytModule = class LichSuXacNhanBhytModule {
};
...
let LichSuXacNhanBHYTService = class LichSuXacNhanBHYTService extends src_app_core_services_base_service__WEBPACK_IMPORTED_MODULE_2__["BaseService"] {
    constructor(apiService) {
        super(apiService);
        this.controllerName = 'LichSuXacNhanBHYT';
    }
};
LichSuXacNhanBHYTService.ctorParameters = () => [
...
let ThongTinPhieuBHYTPdfPopupComponent = class ThongTinPhieuBHYTPdfPopupComponent {
    ...
    getALlFilePDFFromHtml(arrHtml) {
        ...
        arrHtml.forEach((res) => {
            ...
            this.apiService.postExportPdf("XacNhanBHYT/GetFilePDFFromHtml", obj).subscribe((file) => {
...
ThongTinPhieuBHYTPdfPopupComponent.ctorParameters = () => [
