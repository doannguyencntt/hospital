...
let ChonLoaiKetQuaXetNghiemNewPopupComponent = class ChonLoaiKetQuaXetNghiemNewPopupComponent {
    constructor(data, sanitizer, dialog, dialogRef, apiService, notificationService, cdRef) {
        this.data = data;
    ...
    timKiemGridDataSource(searching) {
        if (searching == undefined && searching == null && searching == "") {
            ...
            if (this.gridDataSource.data.length != 0) {
                ...
                this.apiService.post("XetNghiem/TimKiemGridPopUpXetNghiem", modelSearch).subscribe((result) => {
...
ChonLoaiKetQuaXetNghiemNewPopupComponent.ctorParameters = () => [
...
let DichVuCanCapNhatPopupComponent = class DichVuCanCapNhatPopupComponent {
    constructor(ref, notificationService, apiService, dialog, authService, data, dialogRef) {
        this.ref = ref;
        this.notificationService = notificationService;
        this.apiService = apiService;
        this.dialog = dialog;
    ...
    luu() {
        ...
        if (self.authService.hasPermission(self.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_11__["SecurityOperation"].Update)) {
            ...
            self.apiService.post("DuyetKetQuaXetNghiem/CapNhatDichVuXetNghiem", self.duyetKqXetNghiemDetail).subscribe(() => {
                self.closepopupSavingData();
                let mess = src_app_core_utilities_common_helper__WEBPACK_IMPORTED_MODULE_6__["CommonService"].format(src_app_shared_configdata_system_message__WEBPACK_IMPORTED_MODULE_8__["SystemMessage"].ActionSuccessfully, ["Cập nhật"]);
...
DichVuCanCapNhatPopupComponent.ctorParameters = () => [
    { type: _angular_core__WEBPACK_IMPORTED_MODULE_1__["ChangeDetectorRef"] },
...
let DuyetKetQuaInNhomDvListNewPopupComponent = class DuyetKetQuaInNhomDvListNewPopupComponent {
    constructor(sanitizer, dialog, dialogRef, data) {
        this.sanitizer = sanitizer;
        this.dialog = dialog;
        this.dialogRef = dialogRef;
        this.data = data;
...
DuyetKetQuaInNhomDvListNewPopupComponent.ctorParameters = () => [
    { type: _angular_platform_browser__WEBPACK_IMPORTED_MODULE_4__["DomSanitizer"] },
    { type: _angular_material_dialog__WEBPACK_IMPORTED_MODULE_2__["MatDialog"] },
    { type: _angular_material_dialog__WEBPACK_IMPORTED_MODULE_2__["MatDialogRef"] },
...
let DuyetKetQuaNewDetailedComponent = class DuyetKetQuaNewDetailedComponent {
    constructor(router, route, notificationService, dialog, baseService, authService, apiService, errorService, cdRef, renderer) {
        this.router = router;
        this.route = route;
        this.notificationService = notificationService;
        this.dialog = dialog;
    ...
    GetById(id) {
        ...
        this.baseService.getById(id).subscribe(resultData => {
            ...
            let nhomIds = [];
            if (resultData) {
                this.duyetKqXetNghiemDetail = Object.assign({}, resultData);
                ...
                dialogRef.afterClosed().subscribe(result => {
                    if (result) {
                        if (result != undefined && result != null && result != "") {
                            if (result === 'Yes') {
                                this.savingPage();
                                this.apiService.post('DuyetKetQuaXetNghiem/DuyetKetQuaXetNghiem', this.duyetKqXetNghiemDetail).subscribe(() => {
                                    this.closepopupSavingData();
                                    this.GetById(this.id);
                                    this.duyetKqXetNghiemDetail.TrangThai = null;
                                    this.notificationService.showSuccess(src_app_core_utilities_common_helper__WEBPACK_IMPORTED_MODULE_12__["CommonService"].format(src_app_shared_configdata_system_message__WEBPACK_IMPORTED_MODULE_11__["SystemMessage"].ActionSuccessfully, ['Duyệt']));
                                    // location.reload();
                                }, (err) => {
                                    this.validationErrors = err.ValidationErrors;
                        ...
                        if (result != undefined && result != null && result != "") {
                            if (result === 'Yes') {
                                ...
                                this.apiService.post('DuyetKetQuaXetNghiem/DuyetKetQuaXetNghiem', this.duyetKqXetNghiemDetail).subscribe(() => {
                                    ...
                                    this.apiService.postExportPdf('XetNghiem/GetAllHtmlPdfDuyetKetQuaXetNghiemDuocCheckGrid', this.inPhieuDuyetKetQua).subscribe(res => {
    ...
    setDataSourceForGrid() {
        ...
        if (this.searchString == null || this.searchString === '') {
            ...
            const search = src_app_core_utilities_common_helper__WEBPACK_IMPORTED_MODULE_12__["CommonService"].removeVietnamese(this.searchString).toLowerCase().trim().replace(/đ/g, 'd');
            this.ChiTietKetQuaXetNghiems = [...this.duyetKqXetNghiemDetail.ChiTietKetQuaXetNghiems].filter(o => (o.GiaTriCu != null && src_app_core_utilities_common_helper__WEBPACK_IMPORTED_MODULE_12__["CommonService"].removeVietnamese(o.GiaTriCu).toLowerCase().replace(/đ/g, 'd')
                .indexOf(search) >= 0)
    ...
    checkBoxDichVu(dataItem, event) {
        ...
        lstDataCurrentParent[0].IdChilds.forEach((parentId) => {
    ...
    showLoading(title = 'Đang lưu dữ liệu...') {
...
DuyetKetQuaNewDetailedComponent.ctorParameters = () => [
...
let DuyetKetQuaNewListComponent = class DuyetKetQuaNewListComponent {
    constructor(router, dialog, apiService, authService, route, renderer, notificationService) {
        this.router = router;
        this.dialog = dialog;
        this.apiService = apiService;
        this.authService = authService;
    ...
    exportExcel() {
        ...
        if (self.authService.hasPermission(self.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_11__["SecurityOperation"].Process)) {
            ...
            self.apiService.postExportExcel('DuyetKetQuaXetNghiem/ExportDuyetKetQuaXetNghiem', self.gridParent._gridQueryInfo).subscribe(resultData => {
    ...
    nhomDichVuCanIn(phienXetNghiemId, yeuCauTiepNhanId) {
        var self = this;
        if (self.authService.hasPermission(self.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_11__["SecurityOperation"].Process)) {
            self.apiService.get("DuyetKetQuaXetNghiem/GetNhomDichVuDuyet?phienXetNghiemId=" + phienXetNghiemId).subscribe(resultData => {
                if (resultData != undefined && resultData != null) {
...
DuyetKetQuaNewListComponent.ctorParameters = () => [
...
let DuyetKetQuaNewPopupComponent = class DuyetKetQuaNewPopupComponent {
...
DuyetKetQuaNewPopupComponent.ctorParameters = () => [
...
let DuyetKetQuaNewRoutingModule = class DuyetKetQuaNewRoutingModule {
};
...
let DuyetKetQuaNewModule = class DuyetKetQuaNewModule {
};
...
let DuyetKetQuaNewService = class DuyetKetQuaNewService extends src_app_core_services_base_service__WEBPACK_IMPORTED_MODULE_2__["BaseService"] {
    constructor(apiService) {
        super(apiService);
        this.controllerName = 'DuyetKetQuaXetNghiem';
    }
};
DuyetKetQuaNewService.ctorParameters = () => [
...
let DuyetKetQuaNhomDvListNewPopupComponent = class DuyetKetQuaNhomDvListNewPopupComponent {
    constructor(dialogRef, apiService, notificationService, data, dialog, ref, authService) {
        this.dialogRef = dialogRef;
        this.apiService = apiService;
        this.notificationService = notificationService;
        this.data = data;
    ...
    print(id, yeuCauTiepNhanId) {
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_9__["SecurityOperation"].Process)) {
            ...
            this.apiService.post('XetNghiem/GetAllHtmlPdfDuyetKetQuaXetNghiemDuocCheckGrid', this.inPhieuDuyetKetQua).subscribe(arrHtml => {
                this.dialog.open(_duyet_ket_qua_phieu_new_popup_duyet_ket_qua_phieu_new_popup_component__WEBPACK_IMPORTED_MODULE_13__["DuyetKetQuaPhieuNewPopupComponent"], {
                    disableClose: false,
                    width: '1000px',
                    data: arrHtml
                }).afterClosed().subscribe(() => {
    ...
    ok() {
        ...
        this.dialogRef.close();
        this.apiService.post("XetNghiem/XemPhieuXetNghiem", this.inPhieuDuyetKetQua).subscribe(resData => {
            if (resData != undefined && resData != null && resData.length > 0) {
                this.closePopupLoadingData();
                this.dialog.open(_duyet_ket_qua_in_nhom_dv_list_new_popup_duyet_ket_qua_in_nhom_dv_list_new_popup_component__WEBPACK_IMPORTED_MODULE_12__["DuyetKetQuaInNhomDvListNewPopupComponent"], {
                    disableClose: false,
                    width: '1200px',
                    data: resData[0].Html,
                }).afterClosed().subscribe(() => {
...
DuyetKetQuaNhomDvListNewPopupComponent.ctorParameters = () => [
...
let DuyetKetQuaPhieuNewPopupComponent = class DuyetKetQuaPhieuNewPopupComponent {
    ...
    getALlFilePDFFromHtml(arrHtml) {
        this.showPopupLoadingData();
        const newBlob = new Blob([arrHtml], { type: 'application/pdf' });
...
DuyetKetQuaPhieuNewPopupComponent.ctorParameters = () => [
    { type: undefined, decorators: [{ type: _angular_core__WEBPACK_IMPORTED_MODULE_1__["Inject"], args: [_angular_material__WEBPACK_IMPORTED_MODULE_2__["MAT_DIALOG_DATA"],] }] },
    { type: src_app_core_services_api_service__WEBPACK_IMPORTED_MODULE_4__["ApiService"] },
    { type: _angular_material__WEBPACK_IMPORTED_MODULE_2__["MatDialog"] },
    { type: src_app_core_services_notification_service__WEBPACK_IMPORTED_MODULE_5__["NotificationService"] }
