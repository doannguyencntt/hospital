...
let DuyetNhapKhoChiTietComponent = class DuyetNhapKhoChiTietComponent {
    ...
    duyet() {
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_10__["SecurityOperation"].Update)) {
            ...
            this.confrim = this.dialog.open(src_app_shared_component_dialogs_confirm_confirm_component__WEBPACK_IMPORTED_MODULE_11__["ConfirmComponent"], {
                ...
                if (result == "Yes") {
                    this.apiService.post("KeToan/DuyetDuocPhamNhapKho/" + this.yeuCauNhapKhoDuocPhamId)
                        .subscribe((resultData) => {
                        if (resultData !== null && resultData !== undefined) {
                            if (resultData === "") {
                                this.BackToList();
                    ...
                    this.apiService.post("KeToan/DuyetDuocPhamNhapKho/" + this.yeuCauNhapKhoDuocPhamId)
                        ...
                        if (resultData !== null && resultData !== undefined) {
                            if (resultData === "") {
                                this.loadingPage();
                                this.notificationService.showSuccess("Duyệt thành công.");
                                this.apiService.post("NhapKhoDuocPham/InPhieuYeuCauNhapKhoDuocPham?yeuCauNhapKhoDuocPhamId=" + this.yeuCauNhapKhoDuocPhamId).subscribe(resData => {
                                    if (resData != undefined && resData != null) {
                                        this.dialog.open(_nhap_xuat_duoc_pham_nhap_kho_phieu_nhap_kho_duoc_pham_popup_phieu_nhap_kho_duoc_pham_popup_component__WEBPACK_IMPORTED_MODULE_14__["PhieuNhapKhoDuocPhamPopupComponent"], {
                                            disableClose: false,
                                            width: '1200px',
                                            data: resData,
        ...
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_10__["SecurityOperation"].Update)) {
            ...
            this.confrim = this.dialog.open(_tu_choi_duyet_duoc_pham_tu_choi_duyet_duoc_pham_component__WEBPACK_IMPORTED_MODULE_9__["TuChoiDuyetDuocPhamComponent"], {
                ...
                if (result != "" && result != undefined) {
                    ...
                    this.apiService.post("KeToan/TuChoiDuyetDuocPhamNhapKho", this.thongTinLyDoHuyNhapKhoDuocPham)
                        .subscribe((resultData) => {
                        if (resultData !== null && resultData !== undefined) {
                            if (resultData) {
...
DuyetNhapKhoChiTietComponent.ctorParameters = () => [
...
let DuyetNhapKhoListComponent = class DuyetNhapKhoListComponent {
    constructor(authService, cd, dialog, elem, notificationService, location, apiService, router, route) {
        this.authService = authService;
        this.cd = cd;
        this.dialog = dialog;
        this.elem = elem;
    ...
    exportExcel() {
        ...
        if (self.authService.hasPermission(self.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_8__["SecurityOperation"].Process)) {
            self.apiService.postExportExcel('KeToan/ExportDuyetDuocPham', self.gridCha._gridQueryInfo).subscribe(resultData => {
    ...
    duyetLai(id) {
        var self = this;
        self.loadingPage();
        self.apiService.post("KeToan/HuyDuyetNhapKhoDuocPham?yeuCauNhapKhoDuocPhamId=" + id).subscribe(() => {
...
DuyetNhapKhoListComponent.ctorParameters = () => [
...
let DuyetNhapKhoRoutingModule = class DuyetNhapKhoRoutingModule {
};
...
let DuyetNhapKhoModule = class DuyetNhapKhoModule {
};
...
let DuyetNhapKhoService = class DuyetNhapKhoService extends src_app_core_services_base_service__WEBPACK_IMPORTED_MODULE_2__["BaseService"] {
    constructor(apiService) {
        super(apiService);
        this.controllerName = 'NhapKhoDuocPham';
    }
};
DuyetNhapKhoService.ctorParameters = () => [
...
let TuChoiDuyetDuocPhamComponent = class TuChoiDuyetDuocPhamComponent {
...
TuChoiDuyetDuocPhamComponent.ctorParameters = () => [
