...
let DuocPhamDaHetHanListComponent = class DuocPhamDaHetHanListComponent {
    ...
    getSharedPrintForm() {
        ...
        return new Promise(resolve => {
            this.apiService.get('DuocPhamDaHetHan/InDanhMucDaHetHan?search=' + queryString).subscribe(resultData => {
                resolve(resultData);
            });
        });
    }
    showPopup() {
        const dialogRef = this.dialog.open(_duoc_pham_da_het_han_popup_duoc_pham_da_het_han_popup_component__WEBPACK_IMPORTED_MODULE_16__["DuocPhamDaHetHanPopupComponent"], {
            width: '1500px',
    ...
    exportExcel() {
        ...
        if (self.authService.hasPermission(self.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_12__["SecurityOperation"].Process)) {
            self.apiService.postExportExcel('DuocPhamDaHetHan/ExportDuocPhamDaHetHan', self.gridChild._gridQueryInfo).subscribe(resultData => {
...
DuocPhamDaHetHanListComponent.ctorParameters = () => [
...
let DuocPhamDaHetHanPopupComponent = class DuocPhamDaHetHanPopupComponent {
    constructor(dialogRef, apiService, data, notificationService) {
    ...
    GetHTML() {
        ...
        const queryString = '{"KhoId":' + KhoQueryString + ',"DuocPham":' + SearchQueryString + '}';
        this.apiService.post('DuocPhamDaHetHan/GetHTML?search=' + queryString).subscribe(resultData => {
            if (resultData != null) {
...
DuocPhamDaHetHanPopupComponent.ctorParameters = () => [
...
let DuocPhamDaHetHanRoutingModule = class DuocPhamDaHetHanRoutingModule {
};
...
class SearchDuocPham {
    constructor(KhoId = null, searchString = null) {
        this.KhoId = KhoId;
...
let DuocPhamDaHetHanModule = class DuocPhamDaHetHanModule {
};
...
let DuocPhamDaHetHanService = class DuocPhamDaHetHanService extends src_app_core_services_base_service__WEBPACK_IMPORTED_MODULE_2__["BaseService"] {
    constructor(apiService) {
        super(apiService);
        this.controllerName = 'DuocPhamDaHetHan';
    }
};
DuocPhamDaHetHanService.ctorParameters = () => [
