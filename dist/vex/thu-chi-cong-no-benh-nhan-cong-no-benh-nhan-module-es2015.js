...
let CongNoBenhNhanChiTietComponent = class CongNoBenhNhanChiTietComponent {
    constructor(route, dialog, router, authService, notificationService, apiService) {
        this.route = route;
        this.dialog = dialog;
        this.router = router;
    ...
    getThongTinHanhChinhBenhNhan() {
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_6__["SecurityOperation"].View)) {
            ...
            this.apiService.get(`CongNoBenhNhan/GetCongNoBenhNhanTTHC?benhNhanId=${this.benhNhanId}`).subscribe((res) => {
    ...
    xemSoPhieuThu(taiKhoanBenhNhanThuId) {
        ...
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_6__["SecurityOperation"].Process)) {
            this.apiService.postExportPdf(`CongNoBenhNhan/GetHTMLPhieuThuNoBenhNhan?taiKhoanBenhNhanThuId=${taiKhoanBenhNhanThuId}&hostingName=${hostingName}`).subscribe((file) => {
...
CongNoBenhNhanChiTietComponent.ctorParameters = () => [
...
let CongNoBenhNhanChuaThuComponent = class CongNoBenhNhanChuaThuComponent {
    constructor(dialog, authService, notificationService, apiService) {
        this.dialog = dialog;
        this.authService = authService;
        this.notificationService = notificationService;
        this.apiService = apiService;
    ...
    xemDanhSachDaThu(taiKhoanBenhNhanThuId) {
        ...
        let hostingName = window.location.protocol + '//' + window.location.host;
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_5__["SecurityOperation"].Process)) {
            this.apiService.postExportPdf(`CongNoBenhNhan/GetHtmlTatCaPhieuThuNoBenhNhan?taiKhoanBenhNhanThuChinhId=${taiKhoanBenhNhanThuId}&hostingName=${hostingName}`).subscribe((file) => {
                let newBlob = new Blob([file], { type: "application/pdf" });
                let datalocalURL = window.URL.createObjectURL(newBlob);
                arrFileUrl.push(datalocalURL);
                arrFilePdf.push(file);
...
CongNoBenhNhanChuaThuComponent.ctorParameters = () => [
    { type: _angular_material__WEBPACK_IMPORTED_MODULE_8__["MatDialog"] },
...
let CongNoBenhNhanDaThuComponent = class CongNoBenhNhanDaThuComponent {
    constructor(dialog, authService, notificationService, apiService) {
        this.dialog = dialog;
        this.authService = authService;
        this.notificationService = notificationService;
...
CongNoBenhNhanDaThuComponent.ctorParameters = () => [
    { type: _angular_material__WEBPACK_IMPORTED_MODULE_8__["MatDialog"] },
    { type: src_app_core_services_auth_service__WEBPACK_IMPORTED_MODULE_3__["AuthService"] },
    { type: src_app_core_services_notification_service__WEBPACK_IMPORTED_MODULE_4__["NotificationService"] },
    { type: src_app_core_services_api_service__WEBPACK_IMPORTED_MODULE_9__["ApiService"] }
...
let CongNoBenhNhanTraNoPopupComponent = class CongNoBenhNhanTraNoPopupComponent {
    constructor(data, sanitizer, dialog, dialogRef, authService, apiService, notificationService) {
        this.data = data;
    ...
    getThongTinTraNo() {
        if (this.taiKhoanBenhNhanThuId && this.taiKhoanBenhNhanThuId !== 0) {
            if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_8__["SecurityOperation"].View)) {
                this.showLoadingPopup();
                this.apiService.get(`CongNoBenhNhan/GetThongTinTraNo?taiKhoanBenhNhanThuId=${this.taiKhoanBenhNhanThuId}`).subscribe((res) => {
                    if (res) {
        ...
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_8__["SecurityOperation"].Update)) {
            ...
            this.dialog.open(src_app_shared_component_dialogs_confirm_confirm_component__WEBPACK_IMPORTED_MODULE_12__["ConfirmComponent"], {
                ...
                if (res == "Yes") {
                    ...
                    this.apiService.post(`CongNoBenhNhan/ThuTienTraNo`, this.traNo).subscribe((taiKhoanBenhNhanThuId) => {
...
CongNoBenhNhanTraNoPopupComponent.ctorParameters = () => [
...
let CongNoBenhNhanConNoComponent = class CongNoBenhNhanConNoComponent {
    constructor(location, router, authService, notificationService) {
...
CongNoBenhNhanConNoComponent.ctorParameters = () => [
...
let CongNoBenhNhanHetNoComponent = class CongNoBenhNhanHetNoComponent {
    constructor(location, router, authService, notificationService) {
        this.location = location;
        this.router = router;
        this.authService = authService;
...
CongNoBenhNhanHetNoComponent.ctorParameters = () => [
    { type: _angular_common__WEBPACK_IMPORTED_MODULE_3__["Location"] },
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_4__["Router"] },
    { type: src_app_core_services_auth_service__WEBPACK_IMPORTED_MODULE_7__["AuthService"] },
    { type: src_app_core_services_notification_service__WEBPACK_IMPORTED_MODULE_8__["NotificationService"] }
...
let CongNoBenhNhanListComponent = class CongNoBenhNhanListComponent {
    ...
    exportExcel() {
        ...
        if (this.authService.hasPermission(this.documentType, src_app_shared_enum_security_operation_enum__WEBPACK_IMPORTED_MODULE_12__["SecurityOperation"].Process)) {
            ...
            let gridQueryInfo = new src_app_shared_models_common_model__WEBPACK_IMPORTED_MODULE_17__["GridQueryInfo"](0, 10, 10, searchText, null, [{ Field: 'MaBenhNhan', Dir: 'asc' }]);
            this.apiService.postExportExcel(`CongNoBenhnhan/ExportCongNoBenhNhan?isConNo=${isConNo}`, gridQueryInfo).subscribe(res => {
                this.closeAllDialogs();
                this.congNoBenhNhanSearch.listTabCongNoBenhNhan;
                const dateTimeNow = new Date();
                src_app_core_utilities_common_helper__WEBPACK_IMPORTED_MODULE_16__["CommonService"].downLoadFile(res, 'application/vnd.ms-excel', `${isConNo ? 'CongNoBenhNhan-ConNo' : 'CongNoBenhNhan-HetNo'}` + dateTimeNow.getFullYear() + '.xlsx');
            }, (err) => {
                this.notificationService.showError(err.Message);
            });
        }
...
CongNoBenhNhanListComponent.ctorParameters = () => [
...
let CongNoBenhNhanRoutingModule = class CongNoBenhNhanRoutingModule {
};
...
let CongNoBenhNhanService = class CongNoBenhNhanService extends src_app_core_services_base_service__WEBPACK_IMPORTED_MODULE_3__["BaseService"] {
    constructor(apiService) {
        super(apiService);
        this.controllerName = 'CongNoBenhNhan';
    }
};
CongNoBenhNhanService.ctorParameters = () => [
...
class ListTabCongNoBenhNhan {
    constructor(tabConNoSelected = true, tabHetNoSelected = false) {
        this.tabConNoSelected = tabConNoSelected;
        this.tabHetNoSelected = tabHetNoSelected;
    }
}
class ListTabLichSuThuCongNoBenhNhan {
    constructor(tabChuaThuSelected = true, tabDaThuSelected = false) {
        this.tabChuaThuSelected = tabChuaThuSelected;
        this.tabDaThuSelected = tabDaThuSelected;
    }
}
class CongNoBenhNhanSearch {
    constructor(searchString = '', listTabCongNoBenhNhan = new ListTabCongNoBenhNhan()) {
        this.searchString = searchString;
        this.listTabCongNoBenhNhan = listTabCongNoBenhNhan;
    }
}
class CongNoBenhNhanThongTinHanhChinh {
    constructor(Id = 0, MaBenhNhan = null, HoTen = null, GioiTinh = null, GioiTinhDisplay = null, NamSinh = null, SoDienThoai = null, DiaChi = null, ConNo = false) {
        this.Id = Id;
...
class TraNo {
    constructor(Id = 0, TienMat = 0, ChuyenKhoan = 0, POS = 0, NgayThu = null, NoiDungThu = null, TienBenhNhanDua = 0, TienTraLaiBenhNhan = 0) {
        this.Id = Id;
...
let CongNoBenhNhanModule = class CongNoBenhNhanModule {
};
