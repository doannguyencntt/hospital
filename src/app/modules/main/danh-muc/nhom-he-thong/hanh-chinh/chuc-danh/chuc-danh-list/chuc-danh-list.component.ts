...
@Component({
  ...
  templateUrl: './chuc-danh-list.component.html',
  styleUrls: ['./chuc-danh-list.component.scss']
})
export class ChucDanhListComponent implements OnInit {
  ...
  ngOnInit() {
    this.documentType = DocumentType.DanhMucChucDanh;
    this.gridColumns = [
  ...
  updateChucDanh(id: number, isDisabled: boolean) {
  ...
  exportExcel() {
    ...
    if(this.authService.hasPermission(this.documentType, SecurityOperation.Process)) {
      this.apiService.postExportExcel<string>("ChucDanh/ExportChucDanh", this.gridChild._gridQueryInfo).subscribe(res => {
        let dateTimeNow = new Date();
        CommonService.downLoadFile(res, "application/vnd.ms-excel", "ChucDanh" + dateTimeNow.getFullYear() + ".xlsx");
