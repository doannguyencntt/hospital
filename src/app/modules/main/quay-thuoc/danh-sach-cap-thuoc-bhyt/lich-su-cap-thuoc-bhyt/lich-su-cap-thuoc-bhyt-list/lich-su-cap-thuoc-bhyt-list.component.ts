...
import { ExportQueryInfoQueryInfo, QuayXuatThuoc, TagItem } from '../lich-su-cap-thuoc-bhyt.model';
@Component({
    selector: 'app-lich-su-cap-thuoc-bhyt-list',
    templateUrl: './lich-su-cap-thuoc-bhyt-list.component.html',
    styleUrls: ['./lich-su-cap-thuoc-bhyt-list.component.scss']
})
export class LichSuCapThuocBHYTListComponent implements OnInit {
    ...
    additionalSearchStringFrist: string = null;

    @Input() id: number = null;
    @Input() url1: string = null;
    @Input() url2: string = null;

    urlGetDataGridChild: string = "QuayThuoc/GetDanhSachLichSuCapThuocBHYT";
    urlGetTotalPageGridChild: string = "QuayThuoc/GetTotalSachLichSuCapThuocBHYT";
    ...
    exportExcel() {
        ...
        if (self.authService.hasPermission(self.documentType, SecurityOperation.Process)) {
            self.apiService.postExportExcel<string>('QuayThuoc/ExportLichSuCapThuocBHYT',
                self.exportQueryInfoQueryInfo).subscribe(
                    resultData => {
                        ...
                        CommonService.downLoadFile(resultData, 'application/vnd.ms-excel',
                            'LichSuCapThuocBHYT' + dateTimeNow.getFullYear() + '.xlsx');
                    },
                    (err: any) => {
