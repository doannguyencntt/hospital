...
import { BaoCaoTonKhoSearch, ThongTinChiTietXetNghiem } from '../phieu-xuat-hoa-chat-xet-nghiem.model';

@Component({
  selector: 'app-phieu-xuat-hoa-chat-xet-nghiem-list',
  templateUrl: './phieu-xuat-hoa-chat-xet-nghiem-list.component.html',
  styleUrls: ['./phieu-xuat-hoa-chat-xet-nghiem-list.component.scss']
})

export class BaoCaoPhieuXuatHoaChatXetNghiemListComponent implements OnInit {
  ...
  XemChiTiet(dataItem: any) {
    ...
    self.dialog.open(PhieuXuatHoaChatXetNghiemChiTietPopupComponent, {
      disableClose: true,
      width: '1200px',
  ...
  private loadDanhSachPhieuXuatXetNghiem(skip, take) {   
    this.baoCaoSearch.Skip = skip;
    this.baoCaoSearch.Take = take;

    this.apiService.post<any>("BaoCao/GetDataPhieuXuatHoaChatForGrid", this.baoCaoSearch).subscribe(resultData => {
