...
@Component({
  ...
  templateUrl: './duoc-pham-ton-kho-popup-ton-kho.component.html',
  styleUrls: ['./duoc-pham-ton-kho-popup-ton-kho.component.scss']
})
export class DuocPhamTonKhoPopupTonKhoComponent implements OnInit {
  tonKhoDuocPham: any;
  searchString: any;
  loading: boolean = false;
  icClose = icClose;
  constructor(private dialogRef: MatDialogRef<DuocPhamTonKhoPopupTonKhoComponent>, private apiService: ApiService, private dialog: MatDialog,
  ...
  GetHTML() {
    ...
    this.apiService.post<any>("TonKho/GetTonKhoDuocPhamHTML?search=" + queryString).subscribe(
      resultData => {
        loadingDialog.close();
        if (resultData != null) {
          var html = resultData;
