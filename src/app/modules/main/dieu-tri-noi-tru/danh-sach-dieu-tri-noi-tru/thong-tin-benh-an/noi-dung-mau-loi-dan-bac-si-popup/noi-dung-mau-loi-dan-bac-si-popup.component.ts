...
import { NoiDungMauLoiDanBacSi } from '../thong-tin-benh-an.model';
declare var jQuery: any;

@Component({
  selector: 'appnoi-dung-mau-loi-dan-bac-si-popup',
  templateUrl: './noi-dung-mau-loi-dan-bac-si-popup.component.html',
  styleUrls: ['./noi-dung-mau-loi-dan-bac-si-popup.component.scss']
})
export class NoiDungMauLoiDanBacSiPopupComponent implements OnInit {
  ...
  noiDungMau: NoiDungMauLoiDanBacSi = new NoiDungMauLoiDanBacSi();
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<NoiDungMauLoiDanBacSiPopupComponent>,
  ...
  ngOnInit() {
    ...
    this.urlGetData = "DieuTriNoiTru/GetDataForGridNoiDungMauLoiDanBacSi";
    this.urlGetTotalPage = "DieuTriNoiTru/GetTotalPageForGridNoiDungMauLoiDanBacSi";
    this.loaiBenhAn = this.data.LoaiBenhAn;
  ...
  huy() {
    ...
    this.noiDungMau = new NoiDungMauLoiDanBacSi();
    this.isShow = false;
    ...
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      if (this.isChangeData
        && (this.noiDungMau.Id != 0
          ...
          || (this.noiDungMau.HuongXuLyLoiDanBacSi != null && this.noiDungMau.HuongXuLyLoiDanBacSi != "")
        )) {
        this.dialog
  ...
  xuLyGetData(id: number, loadingData: boolean = true) {
    ...
    this.isChangeData = false;
    this.apiService
      .get<any>("DieuTriNoiTru/GetThongTinNoiDungMauLoiDanBacSi?id=" + id)
      .subscribe(
        (resultData) => {
  ...
  xoa(id: number) {
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Delete)) {
      this.dialog.open(ConfirmComponent, {
        ...
        .subscribe((result) => {
          if (result == "Yes") {
            this.apiService.delete("DieuTriNoiTru/XoaNoiDungMauLoiDanBacSi?id=" + id).subscribe(() => {
              this.gridChild.search();
  ...
  luu(id: number = null, isXem: boolean = false, isThem: boolean = false) {
    this.showPopupLoadingData();
    if (this.authService.hasPermission(this.documentType, SecurityOperation.Update)) {
      this.noiDungMau.LoaiBenhAn = this.data.LoaiBenhAn;

      this.apiService.post<any>("DieuTriNoiTru/LuuNoiDungMauLoiDanBacSi", this.noiDungMau).subscribe(res => {
