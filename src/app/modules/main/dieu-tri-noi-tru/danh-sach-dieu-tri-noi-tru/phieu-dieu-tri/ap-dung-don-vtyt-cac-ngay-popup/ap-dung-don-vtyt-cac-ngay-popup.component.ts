...
@Component({
  selector: 'app-ap-dung-don-vtyt-cac-ngay-popup',
  templateUrl: './ap-dung-don-vtyt-cac-ngay-popup.component.html',
  styleUrls: ['./ap-dung-don-vtyt-cac-ngay-popup.component.scss']
})
export class ApDungDonVtytCacNgayPopupComponent implements OnInit {
  icClose = icClose;
  ...
  lstDateChuaCoDonVTYT: any[] = [];
  lstDateCoDonVTYT: any[] = [];
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ApDungDonVtytCacNgayPopupComponent>
  ) {
  }
  ngOnInit() {
    var self = this;
    ...
    this.noiTruDonVTYTTongHopVo.NoiTruDonVTYTTongHopChiTietVos = [...this.data.thuocs];
    this.ngayDieuTriHienTai = new Date(this.lstYearOrderBy[0].Date);
    this.lstDateChuaCoDonVTYT = self.lstYearOrderBy.filter(item => !item.CoDonVTYTNoiTru);
    this.lstDateCoDonVTYT = self.lstYearOrderBy.filter(item => item.CoDonVTYTNoiTru);
    this.getKhoaId();
    ...
    $("#calendar").on("mousedown", "td", function (e) {
      ...
      if (clickedItem.length > 0) {
        ...
        //   return;
        // }
        if (self.lstDateCoDonVTYT.some(z => new Date(z.Date).getDate() == daySelect.getDate()
  ...
  funcBorder() {
    ...
    $(document).ready(function () {
      $("#calendar td[role='gridcell']").each(function () {
        ...
        if (self.lstDateChuaCoDonVTYT.length > 0) {
          self.lstDateChuaCoDonVTYT.forEach(itemChild => {
  ...
  checkExist(date: Date) {
    if (this.lstDateCoDonVTYT.some(z => new Date(z.Date).getDate() == date.getDate()
  ...
  apDungDonVTYT() {
    ...
    if (self.authService.hasPermission(self.documentType, SecurityOperation.Add)) {
      ...
      if (selectedDatesTemp.length == 0) {
        ...
        self.noiTruDonVTYTTongHopVo.Dates = selectedDatesTemp;
        self.addingPage();
            ...
            if (resultData != null) {
              if (resultData == "OK") {
                ...
                let datas = {
                  ...
                  GhiChu: "Ghi chú: Những VTYT bị tô đỏ vì số lượng tồn không đủ hoặc đã hết và không thêm vào các ngày điều trị được chọn khi áp dụng.",
                };
