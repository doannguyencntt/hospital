...
@Component({
	selector: "common-pdf-popup-ngoai-tru",
	templateUrl: "./common-pdf-popup-ngoai-tru.component.html",
	styleUrls: ["./common-pdf-popup-ngoai-tru.component.scss"],
})

export class CommonPdfPopupNgoaiTruComponent implements OnInit {
	icClose = icClose;
	...
	xemBangKeChoThu() {
		this.apiService.postExportExcel<any>("ThuNgan/XuatBangKeNgoaiTruChoThuExcel", this.danhSachChiPhiThuTamUng)
			.subscribe(res => {
				res.yeuCauTiepNhanId
				this.showLoadingPopup();
				if (res.byteLength != 4) {
					let dateTimeNow = new Date();
					CommonService.downLoadFile(res, "application/vnd.ms-excel", "XuatBangKeNgoaiTruChoThuExcel" + dateTimeNow.getFullYear() + ".xlsx");
				}
		...
		this.apiService.postExportExcel<any>("ThuNgan/XuatBangKeNgoaiTruTrongGoiChoThuExcel", this.danhSachChiPhiThuTamUng)
			.subscribe(res => {
				this.showLoadingPopup();
				if (res.byteLength != 4) {
					let dateTimeNow = new Date();
					CommonService.downLoadFile(res, "application/vnd.ms-excel", "XuatBangKeNgoaiTruTrongGoiChoThuExcel" + dateTimeNow.getFullYear() + ".xlsx");
				}
				this.closeAllDialogs();
			}, err => {
				this.notificationService.showError(err.Message);
	...
	xemBangKeDaThu() {	
		this.apiService.postExportExcel<any>("ThuNgan/XuatBangKeNgoaiTruCoBHYTExcelDaThu?yeuCauTiepNhanId=" + this.yeuCauTiepNhanId)
			.subscribe(res => {
				this.showLoadingPopup();
				if (res.byteLength != 4) {
					let dateTimeNow = new Date();
		...
		this.apiService.postExportExcel<any>("ThuNgan/XuatBangKeNgoaiTruExcelDaThu?yeuCauTiepNhanId=" + this.yeuCauTiepNhanId)
			.subscribe(res => {
				this.showLoadingPopup();
				if (res.byteLength != 4) {
					let dateTimeNow = new Date();
		...
		this.apiService.postExportExcel<any>("ThuNgan/XuatBangKeNgoaiTruTrongGoiDvDaThu?yeuCauTiepNhanId=" + this.yeuCauTiepNhanId)
			.subscribe(res => {
				this.showLoadingPopup();
				if (res.byteLength != 4) {
					let dateTimeNow = new Date();
