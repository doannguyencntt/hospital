...
import {DanhSachChiPhiKhamChuaBenh, DanhSachMienGiamVo, LoaiChietKhau, LoaiMienGiam} from "../../noi-tru/cho-thu-tien/cho-thu-tien.model";

@Component({
	selector: "ap-dung-miem-giam-cho-tung-dv-view-popup",
	templateUrl: "./ap-dung-miem-giam-cho-tung-dv-view-popup.component.html",
	styleUrls: ["./ap-dung-miem-giam-cho-tung-dv-view-popup.component.scss"],
})
export class MiemGiamTheoDVComponent implements OnInit {
	icClose = icClose;
	icAdd = icAdd;
	...
	constructor(@Inject(MAT_DIALOG_DATA) public data: any, public sanitizer: DomSanitizer, private dialogRef: MatDialogRef<any>, private cdRef: ChangeDetectorRef) {}
	...
	soTienTruocKhiMiemGiam() {
		...
		return thanhTien - this.data.dataItem.BHYTThanhToan;
	}
	...
	tinhSoTienMiemGiam(dataItem: DanhSachChiPhiKhamChuaBenh) {
		...
		if (dsMienGiamTheoPhanTram != null && dsMienGiamTheoPhanTram.length > 0) {
			...
			dsMienGiamTheoPhanTram.forEach((item) => {
				...
				item.TongTienMiemGiam = Math.round((item.TiLe * this.tongTienTungDichVu) / 100 * 100)/100;
	...
	XuLyMiemGiamThemChietKhauTheoTiLe(item: any, soTienSauMiemGiam: any) {
		if (item.TiLe) item.TongTienMiemGiam = soTienSauMiemGiam * (item.TiLe / 100);
		else item.TongTienMiemGiam = 0;
	}
	...
	XuLyMiemGiamThemChietKhauTheoSoTien(item: any) {
		if (this.disableSoTien) {
			...
			if (item.SoTien > 0 && item.SoTien < this.tongTienTungDichVu - this.tongTatCaSoTienMg) item.SoTien = item.SoTien;
			else item.SoTien = this.tongTienTungDichVu - this.tongTatCaSoTienMg;
	...
	apDungPhanTram(ev: any, item: any) {
		...
		if (ev) {
			...
			this.resetTiLe();
			this.tinhSoTienMiemGiam(this.data.dataItem);
		} else {
			this.disableInputPhanTram = !ev;
			this.resetTiLe();
			this.tinhSoTienMiemGiam(this.data.dataItem);
