...
import { ApDungKhuyenMaiBenhNhanNoiTru, DanhSachDichVuKhuyenMaiBenhNhanNoiTru } from "../cho-thu-tien.model";
declare var $: any;
...
export class DichVuKhuyenMaiKemTheoGoi implements OnInit {
	...
	danhSachDichVuKhuyenMaiBenhNhan: DanhSachDichVuKhuyenMaiBenhNhanNoiTru[] = [];
	apDungKhuyenMaiBenhNhan = new ApDungKhuyenMaiBenhNhanNoiTru();
	...
	apDung() {
		...
		if (this.authService.hasPermission(this.documentType, SecurityOperation.View)) {
			...
			this.apDungKhuyenMaiBenhNhan.DsKhuyenMaiBenhNhanNoiTrus = this.danhSachDichVuKhuyenMaiBenhNhan;
			
			this.apiService.put(`ThuNganNoiTru/ApDungDichVuKhuyenMai`, this.apDungKhuyenMaiBenhNhan).subscribe((response: any) => {
	...
	extCheckboxSelection(event: any[]) {		
		this.danhSachDichVuKhuyenMaiBenhNhan.forEach((e) => {
			e.DanhSachDichVuTrongGoiKhuyenMaiNoiTrus.forEach((e) => {
				{
			...
			if (event.length > 0) {
				for (let index = 0; index < event.length; index++) {
					e.DanhSachDichVuTrongGoiKhuyenMaiNoiTrus.forEach((element) => {
						{
				...
				if (response != undefined && response != null) {
					...
					setTimeout(() => {
						this.danhSachDichVuKhuyenMaiBenhNhan.forEach((element) => {						
							element.DanhSachDichVuTrongGoiKhuyenMaiNoiTrus.forEach((item) => {
								if (item.DaChon) {
