...
import { DuocPham } from '../../../bao-cao/ton-kho/duoc-pham/duoc-pham.model';
...
export class BhytThongTinXacNhanComponent implements OnInit {
    ...
    urlGetDataGridDaHuong = 'XacNhanBHYT/GetDataForDvHuongBhytAsync';
    urlGetDataGridChuaHuong = 'XacNhanBHYT/GetDataForDvChuaHuongBhytAsync';
    ...
    pageSize = 10;
    take: number;
    IsRequestHuongBhyt = false;
    IsRequestChuaHuongBhyt = false;
    sort: SortDescriptor[] = [{
        field: 'Id',
        dir: 'asc'
    }];
    ...
    @ViewChild('ttKDBHYTChiTraGroupFooterTemplate', { static: true }) ttKDBHYTChiTraGroupFooterTemplate: TemplateRef<any>;
    ...
    @ViewChild('bnThanhKhongDHBHYTFooterTemplate', { static: true }) bnThanhKhongDHBHYTFooterTemplate: TemplateRef<any>;
        ...
        switch (field) {
            case 'ThanhTienBenhVien': {
                if (this.dataHuongBh.data.length !== 0) {
                    return this.dataHuongBh.data.reduce((sum: number, item: GridXacNhanBhyt) => sum + item.ThanhTienBenhVien, 0);
                }
            }
        }
    }

    public totalThanhTienKDHBHYT(field: string, typeGrid: string) {
        switch (field) {
            case 'ThanhTienBenhVien': {
    ...
    public totalTTBHYTChiTraDHBHYT(field: string, typeGrid: string) {
        switch (field) {
            case 'TTBHYTChiTra': {
                if (this.dataHuongBh.data.length !== 0) {
    ...
    public totalTTBHYTChiTraKDHBHYT(field: string, typeGrid: string) {
        switch (field) {
            case 'TTBHYTChiTra': {
                if (this.dataChuaHuongBh.data.length !== 0) {
    ...
    public totalBNConPhaiThanhToanKHBHYT(field: any, typeGrid: string) {
        switch (field) {
            case 'BNThanhToan': {
                if (this.dataChuaHuongBh.data.length !== 0) {
    ...
    ngOnInit() {
        const self = this;
        self.documentType = DocumentType.XacNhanBHYT;
    ...
    SetDataSource(huongBh: boolean = true, khongHuongBh: boolean = true) {
        if (huongBh) {
    ...
    PushToXacNhanBhyt(bhytType: BhytType, dataItem: GridXacNhanBhyt) {
        ...
        if (bhytType === BhytType.ChuaDuocHuong) {
            ...
            if (dataItem.GroupType === NhomGoiDichVu.DichVuKhamBenh && this.dataHuongBh.data.indexOf(dataItem) > 4) {
                this.notificationService.showError
                    (DuyetBhytMessage.MessageErrorMoveValidKhamBenh);
            }
        }
    }
