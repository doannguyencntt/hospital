...
import { LoaiDuocPhamVatTu, TrangThaiDuyetKho, XuatKhoKhacGridDaDuyet, XuatKhoVatTuKhac, YeuCauXuatKhoVatTuChiTiet, YeuCauXuatKhoVatTuChiTietVoSearch, YeuCauXuatKhoVatTuGridVo } from '../xuat-kho-khac-ksnk.model';

@Component({
  selector: 'app-xuat-kho-khac-ksnk-shared',
  templateUrl: './xuat-kho-khac-ksnk-shared.component.html',
  styleUrls: ['./xuat-kho-khac-ksnk-shared.component.scss']
})
export class XuatKhoKhacKSNKSharedComponent implements OnInit {
  ...
  urlGetData = "XuatKhoKhacKSNK/GetDataForGridAsyncVatTuDaChon";
  urlGetTotal = "XuatKhoKhacKSNK/GetTotalPageForGridAsyncVatTuDaChon";

  urlGetDataDaDuyetVT = "XuatKhoKhacKSNK/GetDataForGridChildAsyncDaDuyet";
  urlGetTotalDaDuyetVT = "XuatKhoKhacKSNK/GetTotalPageForGridChildAsyncDaDuyet";

  urlGetDataDaDuyetDP = "XuatKhoKhacKSNK/GetDataForGridChildDuocPhamAsyncDaDuyet";
  urlGetTotalDaDuyetDP = "XuatKhoKhacKSNK/GetTotalPageForGridChildDuocPhamAsyncDaDuyet";
    ...
    this.route.queryParams.subscribe(param => {
      ...
      if (!this.isCreate) {
        if (this.tinhTrang == TrangThaiDuyetKho.DaDuyet) {
          ...
          this.getById(this.id, TrangThaiDuyetKho.DaDuyet, this.loaiDuocPhamVatTu);
        } else {
          this.getById(this.id, TrangThaiDuyetKho.ChuaDuyet, this.loaiDuocPhamVatTu);
        }
      }
  ...
  getById(id: number, trangThaiDuyetKho: TrangThaiDuyetKho, loaiDuocPhamVatTu: LoaiDuocPhamVatTu) {
    var self = this;
    if (loaiDuocPhamVatTu === LoaiDuocPhamVatTu.LoaiVatTu) {
      if (self.authService.hasPermission(self.documentType, SecurityOperation.View)) {
        self.addingPage();
        if (trangThaiDuyetKho === TrangThaiDuyetKho.DaDuyet) {
          self.apiService.get<any>("XuatKhoKhacKSNK/GetXuatKhoVatTuKhac?id=" + id).subscribe(
            resultData => {
              if (resultData != undefined && resultData != null) {
                self.disabled = true;
          ...
          self.apiService.get<any>("XuatKhoKhacKSNK/GetYeuCauXuatKhoVatTuKhac?id=" + id).subscribe(
            resultData => {
              if (resultData != undefined && resultData != null) {              
                self.disabled = true;
                self.closepopupLoadingData();
      ...
      if (self.authService.hasPermission(self.documentType, SecurityOperation.View)) {
        self.addingPage();
        if (trangThaiDuyetKho === TrangThaiDuyetKho.DaDuyet) {
          self.apiService.get<any>("XuatKhoKhacKSNK/GetXuatKhoDuocPhamKhac?id=" + id).subscribe(
            resultData => {
              if (resultData != undefined && resultData != null) {
                self.disabled = true;
                self.closepopupLoadingData();
          ...
          self.apiService.get<any>("XuatKhoKhacKSNK/GetYeuCauXuatKhoDuocPhamKhac?id=" + id).subscribe(
            resultData => {
              if (resultData != undefined && resultData != null) {
                self.disabled = true;
                self.closepopupLoadingData();               
  ...
  dieuChuyenVatTuTheoNhomGrid(dataItem: any, gridNumber: number) {
    ...
    this.addingPage();
    this.apiService.post<any>("XuatKhoKhacKSNK/XuatVatTuTheoNhom", model).subscribe(
      resultData => {
        if (resultData != undefined && resultData != null) {
  ...
  TooltipCustom(dataItem: any) {
    this.apiService.post<any>("XuatKhoKhacKSNK/GetSoLuongTonThucTe", dataItem).subscribe(
      resultData => {
        if (resultData != undefined && resultData != null) {
