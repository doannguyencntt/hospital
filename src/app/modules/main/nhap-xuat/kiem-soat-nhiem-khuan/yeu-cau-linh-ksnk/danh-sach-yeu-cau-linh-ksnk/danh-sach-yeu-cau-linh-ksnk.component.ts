...
import { PhieuLinhKSNKThuong, SearchNgay, ExportQueryInfoQueryInfo, LinhKSNKGridVo, PhieuLinhKSNKThuongs } from '../yeu-cau-linh-ksnk.model';
...
@Component({
  ...
  templateUrl: './danh-sach-yeu-cau-linh-ksnk.component.html',
  styleUrls: ['./danh-sach-yeu-cau-linh-ksnk.component.scss']
})
export class DanhSachYeuCauLinhKSNKComponent implements OnInit {

  documentType: DocumentType;
  inPhieuLinhThuong: PhieuLinhKSNKThuongs = new PhieuLinhKSNKThuongs();
  ...
  gridChildLinhDuTruDPChuaDuyetColumns: any[] = [];
  gridChildLinhDuTruDPDaDuyetColumns: any[] = [];
  ...
  baseRoute: string = "/nhap-xuat/kiem-soat-nhiem-khuan/yeu-cau-linh-ksnk";
  ...
  ngOnInit() {
    this.documentType = DocumentType.DanhSachYeuCauLinhKSNK;
    this.gridChild.sort = [{
      field: 'DuocDuyet',
      dir: 'asc'
    }];
  ...
  InPhieuLinh(data: any) {
    ...
    if (data.LoaiPhieuLinh == 1 || data.LoaiPhieuLinh == 2) { //lĩnh dự trù(dự trù) hoặc lĩnh bù
      ...
      {
        self.apiService.post<any>("YeuCauLinhKSNK/InPhieuLinhThuongKSNK", self.inPhieuLinhThuong).subscribe(
          resData => {
      ...
      {
        self.apiService.post<any>("YeuCauLinhKSNK/InPhieuLinhBuKSNK", self.inPhieuLinhThuong).subscribe(
          resData => {
            if (resData != undefined && resData != null) {
              self.closePopupLoadingDataDaDuyet();
              self.inPhieuLinhThuong.Header = false;
      ...
      this.dialog.open(ConfirmComponent, {
        ...
        if (res == "Yes") {
          ...
          if(data.LoaiDuocPhamHayVatTu == true){
            ...
            this.apiService.post<any>("YeuCauLinhKSNK/HuyItemYeuCauLinhThuoc?id=" + dataItem.Id).subscribe(
              resultData => {
                this.notificationService.showSuccess("Xóa thành công");
                this.TimkiemNangCao();
              },
  ...
  Xem(id: number, loaiPhieuLinh: number,daGui :any,loai:any) {
    switch (loaiPhieuLinh) {
      case EnumLoaiPhieuLinh.LinhDuTru:
        if (this.authService.hasPermission(DocumentType.TaoYeuCauLinhThuongKSNK, SecurityOperation.Update)) {
          this.router.navigate(["/nhap-xuat/kiem-soat-nhiem-khuan/yeu-cau-linh-ksnk/gui-lai-phieu-linh-ksnk-thuong/" + id +"/" + loai +"/" + true], { queryParams: { holdQuery: this.trangThaiCheck } }); break;
        } else {
          this.notificationService.showError(SystemMessage.UnAuthorizedMessage);
        }
      case EnumLoaiPhieuLinh.LinhBu:
        if (this.authService.hasPermission(DocumentType.TaoYeuCauLinhBuKSNK, SecurityOperation.Update)) {
          this.router.navigate(["/nhap-xuat/kiem-soat-nhiem-khuan/yeu-cau-linh-ksnk/gui-lai-phieu-linh-ksnk-bu/" + id+"/"+ loai], { queryParams: { holdQuery: this.trangThaiCheck  } }); break;
        } else {
          this.notificationService.showError(SystemMessage.UnAuthorizedMessage);
        }
    }
  ...
  exportExcel() {
    ...
    self.apiService.postExportExcel<string>('YeuCauLinhKSNK/ExportDanhSachLinhKSNK',
      self.exportQueryInfoQueryInfo).subscribe(
        resultData => {
          self.closePopupLoadingData();
          const dateTimeNow = new Date();
          CommonService.downLoadFile(resultData, 'application/vnd.ms-excel',
            'DanhSachYeuCauLinhKSNK' + dateTimeNow.getFullYear() + '.xlsx');
        },
        (err: any) => {
          self.notificationService.showError(err.Message);
        });
