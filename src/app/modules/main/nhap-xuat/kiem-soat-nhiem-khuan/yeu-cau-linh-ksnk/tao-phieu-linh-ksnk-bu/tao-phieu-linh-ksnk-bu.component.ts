...
import { PhieuLinhKSNKThuong, PhieuLinhKSNKThuongs, YeuCauLinhKSNK } from '../yeu-cau-linh-ksnk.model';
...
@Component({
  ...
  templateUrl: './tao-phieu-linh-ksnk-bu.component.html',
  styleUrls: ['./tao-phieu-linh-ksnk-bu.component.scss'],
  animations: [
    stagger60ms,
    fadeInUp400ms
  ]
})
export class TaoPhieuLinhKSNKBuComponent implements OnInit {

  @ViewChild(PhieuLinhKSNKBuGuiLaiSharedComponent, { static: true }) shared: any;
  ...
  luuPhieuLinhKSNK(daGui: boolean) {
    var self = this;
  ...
  xacNhanTaoPhieuLinh() {
    ...
    if (self.authService.hasPermission(self.documentType, SecurityOperation.Add)) {
      ...
      self.loadingPage();
      self.apiService.post<any>("YeuCauLinhKSNK/GuiPhieuLinhBuKSNK", self.yeuCauLinhBuKSNK).subscribe(
        (result) => {
          if (result != undefined && result != null) {
            ...
            self.dialog.open(ConfirmComponent, {
              ...
              if (res == "Yes") {
                ...
                self.apiService.post<any>("YeuCauLinhKSNK/InPhieuLinhBuKSNK", self.inPhieuLinhThuongs).subscribe(
                  resData => {
      ...
      if (self.yeuCauLinhBuKSNK.YeuCauVatTuBenhViens.every(z => !z.CheckBox)) {
        ...
        self.dialog.open(ConfirmComponent, {
          ...
          if (res == "Yes") {
            ...
            self.apiService.post<any>("YeuCauLinhKSNK/InPhieuLinhBuKSNKXemTruoc", dataIn).subscribe(
              resData => {
                if (resData != undefined && resData != null) {
                  self.closePopupLoadingDataHoanThanh();
                  self.inPhieuLinhThuong.Header = false;
