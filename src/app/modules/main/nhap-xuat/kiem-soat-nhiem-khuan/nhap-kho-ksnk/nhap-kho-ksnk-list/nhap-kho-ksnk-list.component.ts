...
import { NhapKhoVatTuSearch, LookupItemViTriVo, rangeDate, InPhieuNhapKhoVatTu } from '../nhap-kho-ksnk';
...
@Component({
  ...
  templateUrl: './nhap-kho-ksnk-list.component.html',
  styleUrls: ['./nhap-kho-ksnk-list.component.scss']
})
export class NhapKhoKSNKListComponent implements OnInit {
  sort: SortDescriptor[] = [{
    field: 'Id',
    dir: 'desc'
  }];

  baseRoute: string = "/nhap-xuat/ksnk/nhap-kho";
  ...
  urlGetDataGridChild: string = "NhapKhoVatTuKSNK/GetDataForGridChildAsync";
  urlGetTotalPageGridChild: string = "NhNhapKhoVatTuKSNK/GetTotalPageForGridChildAsync";
  ...
  ngOnInit() {
    ...
    if (
      this.holdQuery != null &&
      LocalStorageHelper.getItem('additionalSearchStringNhapKhoKSNK') != null
    ) {
      this.additionalSearchString = LocalStorageHelper.getItem(
        'additionalSearchStringNhapKhoKSNK'
      );
      this.backWithSearch();
      ...
      LocalStorageHelper.setItem('additionalSearchStringNhapKhoKSNK', null);
  ...
  backWithSearch() {
    if (this.route.snapshot.queryParams.holdQuery != undefined) {
      ...
      if (holdQuery != null && holdQuery) {
        var additionalSearchString = LocalStorageHelper.getItem<string>("additionalSearchStringNhapKhoKSNK");
  ...
  exportExcel() {
    ...
    if (self.authService.hasPermission(self.documentType, SecurityOperation.Process)) {
      ...
      self.apiService.postExportExcel<string>('NhapKhoVatTuKSNK/ExportNhapKhoKSNK',
        self.gridChild._gridQueryInfo).subscribe(
          resultData => {
            const dateTimeNow = new Date();
            CommonService.downLoadFile(resultData, 'application/vnd.ms-excel', 'NhapKhoVatTuKSNK' + dateTimeNow.getFullYear() + '.xlsx');
  ...
  inPhieu(id: number) {
    ...
    self.dialog.open(XacNhanChonPhieuInVatTuKSNKPopupComponent, {
      disableClose: false,
      width: "400px",
    ...
    self.dialog.open(ConfirmComponent, {
      ...
      if (res == "Yes") {
        if (self.authService.hasPermission(self.documentType, SecurityOperation.Delete)) {
          this.apiService.post<any>("NhapKhoVatTuKSNK/XoaYeuCauNhapKhoVatTu?id=" + id).subscribe(
            () => {
