...
import { InPhieuNhapKhoVatTu, NhapKhoVatTu } from '../nhap-kho-ksnk';
...
@Component({
  ...
  templateUrl: './nhap-kho-ksnk-create.component.html',
  styleUrls: ['./nhap-kho-ksnk-create.component.scss']
})
export class NhapKhoKSNKCreateComponent implements OnInit {

  nhapKhoVatTu: NhapKhoVatTu = new NhapKhoVatTu();
  inPhieuNhapKhoVatTu: InPhieuNhapKhoVatTu = new InPhieuNhapKhoVatTu();

  documentType: DocumentType = DocumentType.NhapVatTuThuocNhomKSNK;
  validationErrors: any;
  popupLoadingData: any;
  @ViewChild(NhapKhoKSNKShareComponent, { static: true }) shared: any;
    ...
    self.dialog.open(ConfirmComponent, {
      ...
      if (res == "Yes") {
        ...
        if (self.authService.hasPermission(self.documentType, SecurityOperation.Add)) {
          self.loadingPage();
          self.apiService.post<any>("NhapKhoVatTuKSNK/ThemYeuCauNhapKhoVatTu", self.nhapKhoVatTu).subscribe(
            (result) => {
              if (result != undefined && result != null) {
                if (result.length > 0) {
                  self.dialog.open(DsVatTuKSNKTrungNccPopupComponent, {
                    disableClose: false,
                    width: '1200px',
                    data: result,
