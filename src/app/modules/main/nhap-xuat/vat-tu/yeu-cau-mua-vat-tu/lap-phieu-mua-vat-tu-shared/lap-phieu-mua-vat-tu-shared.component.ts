...
import { ThongTinVatTuChiTiet, YeuCauMuaVatTu, YeuCauMuaVatTuChiTiet, YeuCauMuaVatTuChiTietValidator } from '../yeu-cau-mua-vat-tu.model';
...
@Component({
  ...
  templateUrl: './lap-phieu-mua-vat-tu-shared.component.html',
  styleUrls: ['./lap-phieu-mua-vat-tu-shared.component.scss']
})
export class LapPhieuMuaVatTuSharedComponent implements OnInit {
  ...
  chonVatTu(dataItem: any) {
    if (dataItem != undefined && dataItem != null) {
      ...
      this.apiService.post<any>("YeuCauMuaVatTu/ThongTinVatTu", this.vatTuGrid).subscribe(
        resultData => {
          if (resultData != undefined && resultData != null) {
  ...
  themVatTu() {
    ...
    self.apiService.post<ThongTinVatTuChiTiet>("YeuCauMuaVatTu/ThemVatTuChiTietGridVo", self.vatTuGrid).subscribe(
      resultData => {
        if (resultData != undefined && resultData != null) {
  ...
  themVatTuBV() {
    var self = this;
    self.dialog.open(ThemVatTuDuTruMuaPopupComponent, {
      disableClose: false,
      width: '900px',
  ...
  getById(id: number) {
    ...
    if (self.authService.hasPermission(self.documentType, SecurityOperation.View)) {
      self.apiService.get<any>("YeuCauMuaVatTu/GetPhieuMuaVatTuDuTru?id=" + id).subscribe(
        resultData => {
          if (resultData != undefined && resultData != null) {
            //console.log(resultData)
